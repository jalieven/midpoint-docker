...
current_dir=`pwd`/
sed -i -e 's@jdbc\:postgresql\:\/\/postgres_resource\:8432\/pgres@jdbc\:postgresql\:\/\/localhost\:8432\/pgres@g' midpoint-dist/var/post-initial-objects/400-postgresql-webidm-resource.xml
sed -i -e "s@<icscscriptedsql:scriptRoots>/opt/midpoint/var/scripts</icscscriptedsql:scriptRoots>@<icscscriptedsql:scriptRoots>${current_dir//\//\\/}midpoint-dist/var/scripts</icscscriptedsql:scriptRoots>@g" midpoint-dist/var/post-initial-objects/400-postgresql-webidm-resource.xml
sed -i -e 's@<gen67:host>ldap</gen67:host>@<gen67:host>localhost</gen67:host>@g' midpoint-dist/var/post-initial-objects/300-opendj-resource.xml

sed -i -e 's@Starting midPoint...@Starting midPoint... ($JAVA_OPTS)@g'  midpoint-dist/bin/midpoint.sh
